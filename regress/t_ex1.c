...
main(int argc, char **argv)
{
	...
	assert((fsm = myfsm_init(NULL, 0)) != NULL);
	assert(myfsm_advance(fsm, G_DONE1, NULL, 0) == -1);
	assert(myfsm_current(fsm) == STATE_A);
	assert(myfsm_advance(fsm, B_DONE1, NULL, 0) == -1);
	assert(myfsm_advance(fsm, B_DONE2, NULL, 0) == -1);
	assert(myfsm_current(fsm) == STATE_A);
	...
	myfsm_set_precondition_ctx(fsm, (void *)&a_finished_called);
	myfsm_set_transition_ctx(fsm, (void *)&b_finished_called);

	assert(myfsm_advance(fsm, A_DONE, NULL, 0) == -1);
	...
	assert(myfsm_advance(fsm, A_DONE, NULL, 0) == 0);
	...
	assert(myfsm_advance(fsm, B_DONE2, NULL, 0) == 0);
	...
	assert(myfsm_advance(fsm, C_DONE2, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_D4);

	assert(myfsm_advance(fsm, D_DONE2, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_E2);

	assert(myfsm_advance(fsm, E_DONE, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_F);

	assert(f_done1_called == 0);
	assert(f_done1_precondition_called == 0);
	assert(myfsm_advance(fsm, F_DONE1, NULL, 0) == -1);
	assert(myfsm_current(fsm) == STATE_F);
	assert(f_done1_called == 0);
	assert(f_done1_precondition_called == 1);

	assert(myfsm_advance(fsm, F_DONE1, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_G);
	assert(f_done1_called == 1);
	assert(f_done1_precondition_called == 2);

	assert(myfsm_advance(fsm, G_DONE2, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_F);

	assert(myfsm_advance(fsm, F_DONE2, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_B);

	assert(myfsm_advance(fsm, B_DONE1, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_C1);
	assert(myfsm_advance(fsm, C_DONE1, NULL, 0) == 0);
	...
	assert(myfsm_advance(fsm, G_DONE1, NULL, 0) == 0);
	assert(myfsm_current(fsm) == STATE_A);
