...
main(int argc, char **argv)
{
	...
	assert(myfsm_advance(fsm, EXCITED_STATE, NULL, 0) == -1);
	assert(ie_called == 1);
	assert(myfsm_advance(fsm, EXCITED_STATE, NULL, 0) == -1);
	assert(ie_called == 2);
	assert(myfsm_advance(fsm, EXCITED_STATE, NULL, 0) == 0);
	assert(myfsm_current(fsm) == EXCITED_STATE);
	assert(ie_called == 3);
	assert(ne_called == 1);
	assert(rtep_called == 0);
	myfsm_set_precondition_ctx(fsm, (void *)atoi);
	rtep_expected_cb_state = EXCITED_STATE;
	assert(myfsm_advance(fsm, GROUND_STATE, NULL, 0) == -1);
	assert(myfsm_current(fsm) == EXCITED_STATE);
	assert(rtep_called == 1);
	rtep_fail = 0;
	assert(myfsm_advance(fsm, GROUND_STATE, NULL, 0) == 0);
